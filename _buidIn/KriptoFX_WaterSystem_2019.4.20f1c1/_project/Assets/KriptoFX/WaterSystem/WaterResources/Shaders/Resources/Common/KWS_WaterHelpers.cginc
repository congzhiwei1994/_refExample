...
inline half GetWaterRawFade(float3 worldPos, float surfaceDepthZ, float refractedSceneZ, half surfaceMask, out half depthAngleFix)
{
	depthAngleFix = (surfaceMask < 0.5 || KWS_IsCustomMesh == 1) ? 0.25 : saturate(GetWorldSpaceViewDirNorm(worldPos - float3(0, KW_WindSpeed * 0.5, 0)).y) ;
	//depthAngleFix = 1;
