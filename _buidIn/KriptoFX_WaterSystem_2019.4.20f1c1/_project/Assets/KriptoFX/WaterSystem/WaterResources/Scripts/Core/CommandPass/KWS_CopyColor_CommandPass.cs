...
namespace KWS
{
    public class KWS_CopyColor_CommandPass
    {
        ...
        {
            if (copyColorMaterial == null) copyColorMaterial = KWS_CoreUtils.CreateMaterial(KWS_ShaderConstants.ShaderNames.CopyColorShaderName);
        }

        void InitializeTextures()
        {
            if (_isTexturesInitialized) return;
            _colorRT = KWS_RTHandles.Alloc(Vector2.one, name: "_CameraOpaqueTexture", colorFormat: GetGraphicsFormatHDR(), filterMode: FilterMode.Bilinear);
