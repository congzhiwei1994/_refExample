    ...
    public class KW_ShorelineWaves : MonoBehaviour
    {
       ...
       // private GameObject quad;
        ...
        public void InitializeMaterials()
        {
            ...
            {
                foamMaterial = CreateMaterial(KWS_ShaderConstants.ShaderNames.ShorelineFoamParticles_shaderName);
                WaterInstance.AddMaterialToWaterRendering(foamMaterial);
            }
            if (foamShadowMaterial == null)
            {
                foamShadowMaterial = CreateMaterial(KWS_ShaderConstants.ShaderNames.ShorelineFoamParticlesShadow_shaderName);
                WaterInstance.AddMaterialToWaterRendering(foamShadowMaterial);
            }
        }

        public KW_Extensions.AsyncInitializingStatusEnum StatushorelineDataLoadStatus { get; private set; }

        public async Task<ShorelineData> GetShorelineData(string GUID)
        {
            if (StatushorelineDataLoadStatus == KW_Extensions.AsyncInitializingStatusEnum.NonInitialized)
            {
                StatushorelineDataLoadStatus = KW_Extensions.AsyncInitializingStatusEnum.StartedInitialize;
                ...
                {
                    StatushorelineDataLoadStatus = KW_Extensions.AsyncInitializingStatusEnum.Failed;
                ...
                if (_shorelineData == null)
                {
                    StatushorelineDataLoadStatus = KW_Extensions.AsyncInitializingStatusEnum.Failed;
                    _shorelineData           = new ShorelineData();
                    return _shorelineData;
                }
                StatushorelineDataLoadStatus = KW_Extensions.AsyncInitializingStatusEnum.Initialized;
            }
        ...
        public void UpdateLodLevels(Vector3 camPos, int qualityLevel, bool foamCastShadows, bool foamReceiveShadows)
        {
            ...
            foreach (var vatLodGO in foamGameObjects)
            {
                if (vatLodGO.Value != null)
                {
                    ...
                    int n = qualityLevel;
                    for (var i = qualityLevel; i < lods.Length; i++)
                    {
                        float lodDist = 100f;
                        switch (qualityLevel)
                        {
                            case 0:
                                lodDist = LodDistances_High[i];
                                break;
                            case 1:
                                lodDist = LodDistances_Medium[i];
                                break;
                            case 2:
                                lodDist = LodDistances_Low[i];
                                break;
                        }
        ...
        void OnDisable()
        {
            ...
            KW_Extensions.SafeDestroy(foamMaterial, foamShadowMaterial, shorelineWaveDisplacementTex, shorelineWaveNormalTex,
                ...
                VAT_Position, VAT_RangeLookup, VAT_Offset);
