...
namespace KWS
{
    public class KW_FluidsSimulation2D : MonoBehaviour
    {
        ...
        private const float AreaSizeMultiplierLod1 = 3;
        ...
        public void SaveOrthoDepth(string GUID, Vector3 position, int areaSize, int texSize)
        {
            ...
            texSize = Mathf.Clamp(texSize, 128, 2048);
            KW_WaterOrthoDepth.RenderAndSaveDepthUsingRaycast(WaterInstance, position, areaSize, texSize, nearPlaneDepth, farPlaneDepth, pathToDepthTex, pathToDepthData);
            isDepthTextureInitialized = false;
        }
        ...
        public void SavePrebakedSimulation(string GUID)
        {
            ...
            tempRT.SaveRenderTextureToFile(Path.Combine(pathToSimDataFolder, path_fluidsFolder, GUID, path_fluidsPrebakedSim), TextureFormat.BC7, false);
        ...
        public void RenderFluids(Camera currentCamera, Vector3 waterPosition, int areaSize, int resolution, float flowSpeed, float foamStrength, string GUID)
        {
            ...
            areaPosition_lod1 += ComputeAreaSimulationJitter(AreaSizeMultiplierLod1 * areaSize / resolution, AreaSizeMultiplierLod1 * areaSize / resolution);
            ...
            Shader.SetGlobalFloat("KW_FluidsRequiredReadPrebakedSim", frameNumber < 10 ? 1 : 0);
            Shader.SetGlobalFloat("KW_FluidsVelocityAreaScale", (0.5f * areaSize / 40f) * flowSpeed);
            Shader.SetGlobalVector("KW_FluidsMapWorldPosition_lod0", areaPosition);
            Shader.SetGlobalVector("KW_FluidsMapWorldPosition_lod1", areaPosition_lod1);
            Shader.SetGlobalFloat("KW_FluidsMapAreaSize_lod0", areaSize);
            Shader.SetGlobalFloat("KW_FluidsMapAreaSize_lod1", areaSize * AreaSizeMultiplierLod1);

            var target_lod1 = RenderFluidLod(fluidsData[0], fluidsData[1], flowSpeed * 0.5f, areaSize * AreaSizeMultiplierLod1, foamStrength * 2.5f, (Vector3)offset_lod1, areaPosition_lod1, false);
