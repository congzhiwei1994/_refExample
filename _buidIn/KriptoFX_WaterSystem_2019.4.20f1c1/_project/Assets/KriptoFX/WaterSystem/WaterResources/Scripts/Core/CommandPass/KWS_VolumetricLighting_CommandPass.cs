...
namespace KWS
{
    public class KWS_VolumetricLighting_CommandPass
    {
        ...
        void CheckAndResetRTHandleSize()
        {
        ...
        void InitializeMaterials()
        {
            ...
            if (volumeLightBlurMat == null) volumeLightBlurMat = CreateMaterial(KWS_ShaderConstants.ShaderNames.BlurBilateralName);
        }
        ...
        {
            if (_waterInstance.VolumetricLightResolutionQuality == WaterSystem.VolumetricLightResolutionQualityEnum.VeryLow
             || _waterInstance.VolumetricLightResolutionQuality == WaterSystem.VolumetricLightResolutionQualityEnum.Low) volumeLightMat.DisableKeyword(WaterKeywords.USE_CAUSTIC);
            else if (_waterInstance.UseCausticEffect) volumeLightMat.EnableKeyword(WaterKeywords.USE_CAUSTIC);
        }
        ...
        private void UpdateMaterialParams(Camera cam)
        {
            ...
            if(_waterInstance.WaterMeshType == WaterSystem.WaterMeshTypeEnum.InfiniteOcean) volumeLightFade = Mathf.Clamp01(Mathf.Exp(-1 * (_waterInstance.transform.position.y - cam.transform.position.y) / _waterInstance.Transparent)); //todo add depth fading relative to the actual surface height
