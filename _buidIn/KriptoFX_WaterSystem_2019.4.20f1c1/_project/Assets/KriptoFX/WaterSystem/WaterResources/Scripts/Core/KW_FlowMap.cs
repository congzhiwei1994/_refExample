...
namespace KWS
{
    public class KW_FlowMap : MonoBehaviour
    {
        ...
        public void SaveFlowMap(int areaSize, string GUID)
        {
            ...
            var pathToDepthDataFolder = KW_Extensions.GetPathToStreamingAssetsFolder();
            _flowmapRT.rt.SaveRenderTextureToFile(Path.Combine(pathToDepthDataFolder, KWS_Settings.DataPaths.FlowmapFolder, GUID, KWS_Settings.DataPaths.FlowmapTexture), TextureFormat.BC5);
        ...
        public async Task<bool> ReadFlowMap(string GUID)
        {
            ...
            if (_loadedFlowmapData == null || _loadedFlowmapData.AreaSize == 0)
            {
                ...
                //Debug.LogWarning("'Flowing' is activated, but the 'flowmap texture' hasn't been initialized yet. Use 'FlowMap Painter' and save the result.");
                return false;
            }
