...
//  参考地址：https://github.com/bad3p/TerrainBlending
//******************************************************
using UnityEngine;
...
public class TerrainShaderPropertyBridge : MonoBehaviour
{
    private static class Uniforms
    {
        ...
        internal static readonly int[] _Mask =
        {
            Shader.PropertyToID("_TerrainMask0"),
            Shader.PropertyToID("_TerrainMask1"),
            Shader.PropertyToID("_TerrainMask2"),
            Shader.PropertyToID("_TerrainMask3"),
        };

        //internal static readonly int[] _Splat =
        //{
        ...
        //internal static readonly int[] _Normal =
        //{
    ...
    Vector4 GetSplatTextureScaleOffset(int terrainLayer)
    {
...
#endif
        {
            ...
            temp = new Vector4(_cachedTerrainSize.x / tileSize.x, _cachedTerrainSize.z / tileSize.y, tileOffset.x, tileOffset.y);
           // return 
        }
        else
        {
            ...
            temp= new Vector4(terrainSize.x / tileSize.x, terrainSize.z / tileSize.y, tileOffset.x, tileOffset.y);
        }

        //if(EditorApplication.isPlaying ==true)
        //Debug.Log("纹理UVTiling/Scale-->"+temp +"--->层级"+terrainLayer);
        return temp;
    }
    ...
    void Update()
    {
...
#endif
        {
            for (int i = 0; i < _cachedTerrainLayers.Length; i++)
            {
                ...
                //Shader.SetGlobalTexture(Uniforms._Splat[i], _cachedTerrainLayers[i].diffuseTexture);
                //Shader.SetGlobalTexture(Uniforms._Normal[i], _cachedTerrainLayers[i].normalMapTexture);
                Shader.SetGlobalTexture(Uniforms._Mask[i], _cachedTerrainLayers[i].maskMapTexture);
        ...
        {
            //Debug.Log("CachedTerrainLength= " + _cachedTerrainLayers.Length);
            for (int i = 0; i < _cachedTerrainLayers.Length; i++)
            {
                //Debug.Log("CachedTerrainIndex = " + i);
