...
{
	[Serializable]
	public class AILWSubShader : IAILWSubShader
	{
		...
		Pass m_ForwardPassMetallic = new Pass
		{
			...
			{
				AILWMasterNode.AlphaThresholdSlotId,
				AILWMasterNode.Buffer0Id,
				AILWMasterNode.Buffer1Id,
				AILWMasterNode.Buffer2Id,
				AILWMasterNode.Buffer3Id,
				AILWMasterNode.Buffer4Id,
				AILWMasterNode.Buffer5Id,
				AILWMasterNode.Buffer6Id,
				AILWMasterNode.Buffer7Id
			},
			VertexShaderSlots = new List<int>()
			{
				AILWMasterNode.PositionSlotId
			}
		};
		...
		static string GetTemplatePath( string templateName, string guid = null )
		{
			var basePath = Path.GetFullPath( "Packages/com.unity.render-pipelines.lightweight/Editor/ShaderGraph" );
			if( !string.IsNullOrEmpty( guid ) )
				basePath = Path.GetFullPath( Path.GetDirectoryName( AssetDatabase.GUIDToAssetPath( guid ) ) );
		...
		static string GetShaderPassFromTemplate( string template, AILWMasterNode masterNode, Pass pass, GenerationMode mode, SurfaceMaterialOptions materialOptions )
		{
			// ----------------------------------------------------- //
			//                         SETUP                         //
			// ----------------------------------------------------- //
			...
			//if( masterNode.IsSlotConnected( AILWMasterNode.NormalSlotId ) )
			//	defines.AppendLine( "#define _NORMALMAP 1" );

			if( masterNode.model == AILWMasterNode.Model.Specular )
				defines.AppendLine( "#define _SPECULAR_SETUP 1" );

			if( masterNode.IsSlotConnected( AILWMasterNode.AlphaThresholdSlotId ) )
