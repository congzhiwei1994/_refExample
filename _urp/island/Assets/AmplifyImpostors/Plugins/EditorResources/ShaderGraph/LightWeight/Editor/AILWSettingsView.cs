...
{
    public class AILWSettingsView : VisualElement
    {
        AILWMasterNode m_Node;
        public AILWSettingsView( AILWMasterNode node )
        {
            m_Node = node;
            ...
            ps.Add(new PropertyRow(new Label("Workflow")), (row) =>
                {
                    row.Add(new EnumField(AILWMasterNode.Model.Metallic), (field) =>
                    {
        ...
        void ChangeWorkFlow(ChangeEvent<Enum> evt)
        {
            ...
            m_Node.model = (AILWMasterNode.Model)evt.newValue;
        }
