Shader "JZPAJ/Character_Aniso_SSS_AO_Tone"
{
    Properties
    {
		...
		_Dir_Tone("_Dir_Tone(Vector3)", Color) = (1.00, 0.6078, 0.2902, 1.0)
		_Dir_Light_Intensity("_Dir_Light_Intensity", Float) = 1.0
		_ChangeAmount("_ChangeAmount", Range(0,1)) = 0.5
		_Env_Tone("_Env_Tone(Vector3)", Color) = (0.8902, 0.5137, 0.5569, 1.00)
...
//				half3 normalTS = UnpackNormalRG_Optimize(normalCol.rg, 1.0, 1);
...
//				
//				half4 hairCol = SAMPLE_TEXTURE2D(_HairMap, sampler_HairMap, input.uv.xy);
//				half3 hairNormalTS = UnpackNormalRG_Optimize(hairCol.rg, 1.0, 0);
//				half3 hairNormalWS = normalize(TransformTangentToWorld(hairNormalTS, half3x3(input.tangentWS.xyz, input.bitangentWS.xyz, input.normalWS.xyz)));
//				half normalOffset = _Normal_Offset + ((hairCol.b - 0.5) * _Noise_Offset);
//
//
...
//
//				half finalNoH = lerp(NoH, max(0.0, sin(saturate(dot(normalize((normalWS * normalOffset) + hairNormalWS), viewDirectionWS)) * PI)), step(mixCol.y, 0.25));
//
...
//				// AO_slider 在1时，AO在0-0.5时，local_694由1-0线性变化其它为0
//				// AO_slider 控制的是local_694由什么到0的线性变化
...
//
//				float3 local_784 = ((finalNoL + local_716) * lightColor);
...
//				float3 env_tone = lerp(_Env_Tone.rgb, 1.0, _ChangeAmount);
//				half3 local_1124 = (local_1123 * env_tone + 0);
//
//				// 边缘光
